...
```{r}
#| label: select_data
type_reviews <- shopping_trends_raw %>%
  select(
    Gender, 
    Review.Rating
  ) %>%
...
```{r}
#| label: vizualization_for_gender_review
ggplot(
  data = type_reviews,
  aes(
    x = Gender,
...
```{r} 
#| label: summaraize_data
shopping_summary <- lm(formula = Purchase.Amount..USD. ~ Category, data = shopping_trends_raw)
summary(shopping_summary)
```
