...
ggplot(
  ...
  labs(
    x = "Gender",
    y = "Review Rating",
    title = "Men vs Women Reviews"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5)
  )
```
We wanted to research the difference of what review ratings men and women give after purchasing an item. We believed that women would be more opinionated with their reviews, leaving both higher and lower ratings. However after looking at this graph we were able to find that men tend to leave higher review ratings then women. The data that we did use has a larger sample of men compared to women which could lead to skewed data. However, with the data we have both men and women have similar ratings with men having a slightly higher percent of good ratings. 
...
## **References**
Mohit, Bhadra.(2024) *Costumer Shopping(Latest Trends)Dataset*. Kaggle. <https://www.kaggle.com/datasets/bhadramohit/customer-shopping-latest-trends-dataset>.
