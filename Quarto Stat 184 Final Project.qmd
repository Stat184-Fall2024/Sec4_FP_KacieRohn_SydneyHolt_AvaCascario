...
locationSeasonData <- shoppingTrendsRaw %>%
  group_by(
    Location, 
    Season
  ) %>%
  summarize(
    Total.Sales = sum(Purchase.Amount..USD.),
    .groups = "drop"
  )

locationSeasonCounts <- shoppingTrendsRaw %>%
  group_by(
    Location, 
    Season
  ) %>%
...
ggplot(
  ...
  labs(
    title = "Sales by Location and Season", 
    x = "Season", 
    y = "Number of Sales", 
    fill = "Location"
  ) +
...
ggplot(
  ...
  labs(
    title = "Items Bought by Location and Season", 
    x = "Item", 
